...
const SellerDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const completedOrders = ordersData.orders?.filter(o => 
        o.orderStatus === 'delivered'
      ).length || 0;
      const totalRevenue = ordersData.orders?.reduce((sum, o) => 
        o.orderStatus === 'delivered' ? sum + o.finalAmount : sum, 0
      ) || 0;

      setStats({
        totalProducts: 0, // Will be updated by ProductManagement component
        activeOrders,
        completedOrders,
        totalRevenue,
  ...
  const handleOrderCancelled = (orderData) => {
    setOrders(prev => prev.map(o => 
      o._id === orderData._id ? { ...o, orderStatus: 'cancelled' } : o
    ));
    setStats(prev => ({
      ...prev,
      activeOrders: Math.max(0, prev.activeOrders - 1)
    }));
    toast.info(`Order #${orderData.orderNumber} was cancelled`);
  };
